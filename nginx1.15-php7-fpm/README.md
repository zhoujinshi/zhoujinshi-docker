...
```console
$ docker run --name cnbbx_nginx -p 80:80 --expose 1935 -p 1935:1935 -v /var/www/html:/var/www/html -v `pwd`/vhosts/:/etc/nginx/conf.d/ -v `pwd`/nginx.conf:/etc/nginx/nginx.conf -d nginx1.15-php7-fpm
```

Alternatively, a simple `Dockerfile` can be used to generate a new image that includes the necessary content (which is a much cleaner solution than the bind mount above):

```dockerfile
FROM nginx1.15-php7-fpm
...
```console
$ docker run --name cnbbx-nginx -d nginx1.15-php7-fpm
```

## Exposing external port

```console
$ docker run --name cnbbx-nginx -d -p 8080:80 nginx1.15-php7-fpm
```
...
```console
$ docker run --name my-custom-nginx-container -v /host/path/nginx.conf:/etc/nginx/nginx.conf:ro -d nginx1.15-php7-fpm
```
...
```console
$ docker run --name tmp-nginx-container -d nginx1.15-php7-fpm
